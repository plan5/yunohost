...
class MyMigration(Migration):
    ...
    def is_pending(self):
        if not self.state:
            self.state = tools_migrations_state()["migrations"].get(
                "0038_rebuild_python_venv_in_trixie", "pending"
            )
    ...
    def disclaimer(self):
        ...
        msg = m18n.n("migration_0038_rebuild_python_venv_in_trixie_disclaimer_base")
        if rebuild_apps:
            msg += "\n\n" + m18n.n(
                "migration_0038_rebuild_python_venv_in_trixie_disclaimer_rebuild",
                rebuild_apps="\n    - " + "\n    - ".join(rebuild_apps),
            )
        if ignored_apps:
            msg += "\n\n" + m18n.n(
                "migration_0038_rebuild_python_venv_in_trixie_disclaimer_ignored",
                ignored_apps="\n    - " + "\n    - ".join(ignored_apps),
            )
    ...
    def run(self):
        ...
        for venv in venvs:
            ...
            if any(
                ...
                logger.info(
                    m18n.n(
                        "migration_0038_rebuild_python_venv_in_trixie_broken_app",
            ...
            logger.info(
                m18n.n(
                    "migration_0038_rebuild_python_venv_in_trixie_in_progress",
                    app=app_corresponding_to_venv,
                )
            )
            ...
            if status != 0:
                logger.error(
                    m18n.n(
                        "migration_0038_rebuild_python_venv_in_trixie_failed",
                        app=app_corresponding_to_venv,
                    )
                )
            else:
